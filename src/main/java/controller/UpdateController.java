...
//	private static final String SQL_INSERT = "INSERT INTO manager (employee, affiliation, job_title) VALUE (?, ?, ?)";
	private static final String SQL_UPDATE = "UPDATE manager SET employee = ?, affiliation = ?, job_title = ? WHERE employee_id = ?";
...
//	private static String JOB_TITLE;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// JDBCドライバー接続
		String message_jdbc = jdbc_driver(JDBC_DRIVER);
		request.setAttribute("message", message_jdbc);
		
		// DB接続 INSERT
		String message_Insert = db_connection_update(employee,affiliation,job_title, employee_id);
...
//		request.setAttribute("message", message_Select);
		...
		// 
		request.setAttribute("message", employee + " さんの更新が完了しました");
		
		// detail.jspにフォワードして表示
		String view = "/detail?employee_id=" + employee_id;
		RequestDispatcher dispatcher = request.getRequestDispatcher(view);
		dispatcher.forward(request, response);
	}
	
	// JDBCドライブに接続
	protected String jdbc_driver(String jdbc_driver) {
	...
	// DB接続 INSERT
	protected String db_connection_update(String employee, String affiliation, String job_title, String employee_id) {
		...
		try(Connection connection = DriverManager.getConnection(JDBC_URL, DB_USER, DB_PASSWORD);
			...
			statement.setString(4,employee_id);
			// 実行
